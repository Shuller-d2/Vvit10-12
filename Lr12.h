...
public:
	...
	//lr12--------------------------------------
	template <typename U>
	friend tlist<U> summ(tlist<U>& vec1, tlist<U>& vec2);
	
	template <typename U>
	friend tlist<U> diff(tlist<U>& vec1, tlist<U>& vec2);
	//function add
	void add(tlist<LT>list) {
		this->copy(summ(*this, list));
	}
	//function substract
	void subtract(tlist<LT>list) {
		this->copy(diff(*this, list));
	}
	template<typename U>
	friend tlist<U> operator+(tlist<U>& list1, tlist<U>& list2);
	template<typename U>
	friend tlist<U> operator-(tlist<U>& list1, tlist<U>& list2);
	//operator dobavlenuy
	tlist<LT>& operator+=(tlist<LT>& list) {
		this->add(list);
		return *this;
	}
	//operator vichitaniy
...
template<typename LT>
//operator slozeniy
tlist<LT> operator+(tlist<LT>& list1, tlist<LT>& list2)
{
	return summ(list1, list2);
}
template<typename LT>
//operator vichitaniy
tlist<LT>operator-(tlist<LT>& list1, tlist<LT>& list2) {
...
template<typename LT>
//function summ
tlist<LT> summ(tlist<LT>& list1, tlist<LT>& list2) {
	auto l1 = list1.begin();
	auto l2 = list2.begin();
	tlist<LT> result;

	for (; l1 != list1.end() && l2 != list2.end(); l1++, l2++) {
...
template<typename LT>
//function didd
tlist<LT> diff(tlist<LT>& list1, tlist<LT>& list2) {
	auto l1 = list1.begin();
	auto l2 = list2.begin();
	tlist<LT> result;
